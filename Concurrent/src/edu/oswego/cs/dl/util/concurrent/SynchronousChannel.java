 ...
 * handoff designs, in which an object running in one thread must synch up with
 * an object running in another thread in order to hand it some information,
 * event, or task.
 * <p>
...
public class SynchronousChannel implements BoundedChannel {
	 ...
	 * 
	 * An arriving putter that does not already have a waiting taker creates a
	 * node holding item, and then waits for a taker to take it. An arriving
	 * putter that does already have a waiting taker fills the slot node created
	 * by the taker, and notifies it to continue.
	 * 
	 * And symmetrically, two for takes:
	 * 
	 * An arriving taker that does not already have a waiting putter creates an
	 * empty slot node, and then waits for a putter to fill it. An arriving
	 * taker that does already have a waiting putter takes item from the node
	 * created by the putter, and notifies it to continue.
	 * 
	 * This requires keeping two simple queues: waitingPuts and waitingTakes.
	 * 
	 * When a put or take waiting for the actions of its counterpart aborts due
	 * to interruption or timeout, it marks the node it created as "CANCELLED",
	 * which causes its counterpart to retry the entire put or take sequence.
	 */

	/**
	 * Special marker used in queue nodes to indicate that the thread waiting
	 * for a change in the node has timed out or been interrupted.
	 **/
	...
	/**
	 * @return null -- Synchronous channels do not hold contents unless actively
	 *         taken
	 **/
	@Override
	public Object peek() {
	...
	public void put(Object x) throws InterruptedException {
		...
		for (;;) {
			...
			synchronized (this) {
				// Try to match up with a waiting taker; fill and signal it
				// below
				slot = this.waitingTakes.deq();

				// If no takers yet, create a node and wait below
				if (slot == null)
					this.waitingPuts.enq(item = new LinkedNode(x));
			}
			...
			if (slot != null) { // There is a waiting taker.
				...
				synchronized (item) {
					try {
						...
						// If item was taken, return normally but set interrupt
						// status
						if (item.value == null) {
	...
	public Object take() throws InterruptedException {
		...
		for (;;) {
			...
			synchronized (this) {
				...
				if (item == null)
					this.waitingTakes.enq(slot = new LinkedNode());
			}
